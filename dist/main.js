...
import { Color, TipoPieza } from './models/types.js';
/* ====================================================================
...
*/
let posicionOrigenSeleccionada = null;
...
function mostrarEfectoCaptura(destino, piezaComida, piezaComedora, colorComedor) {
    /*
    Propósito:
    ...
    */
    ...
    if (colorComedor === Color.BLANCO)
        puntajeBlancas += puntosGanados;
    else
        puntajeNegras += puntosGanados;
    console.log(`[Marcador] Blancas: ${puntajeBlancas} | Negras: ${puntajeNegras}`);
    // Operación CREATE: Texto flotante en el DOM
    const casillaVisual = document.querySelector(`[data-fila="${destino.fila}"][data-columna="${destino.columna}"]`);
    if (casillaVisual) {
...
function evaluarYMostrarPromocion(destino) {
    /*
    Propósito:
    ...
    */
    ...
    if (!pieza || pieza.tipo !== TipoPieza.PEON)
        return;
...
function procesarInteraccionUsuario(filaObjetivo, columnaObjetivo) {
    /*
    Propósito:
    ...
    */
    ...
    const posicionClickeada = { fila: filaObjetivo, columna: columnaObjetivo };
    if (posicionOrigenSeleccionada === null) {
        ...
        try {
            ...
            if (movimientoRealizado) {
                if (datosCaptura) {
                    mostrarEfectoCaptura(datosCaptura.destino, datosCaptura.comida, datosCaptura.comedora, datosCaptura.color);
                }
...
function inicializarTableroVisual() {
    ...
    */
    ...
    if (etiquetaColorLocal)
        etiquetaColorLocal.textContent = `[ ${colorJugadorLocal} ]`;
    if (etiquetaColorRival)
        etiquetaColorRival.textContent = `[ ${colorOponente} ]`;
...
function actualizarPantalla() {
    ...
    */
    ...
    for (let f = 0; f < 8; f++) {
        for (let c = 0; c < 8; c++) {
            const casillaVisual = document.querySelector(`[data-fila="${f}"][data-columna="${c}"]`);
            if (!casillaVisual)
                continue;
            // Operación READ
...
function mostrarEfectoJaque(colorEnJaque) {
    /*
    Propósito:
